...
extern bool writeisb(int fd, uint32_t interfaceid, uint64_t starttimestamp,
		     uint64_t incoming);

extern uint16_t addoption(uint8_t *shb, uint16_t optioncode, uint16_t optionlen,
			  char *option);

extern int hcxcreatepcapngdump(char *pcapngdumpname, uint8_t *macrorig,
