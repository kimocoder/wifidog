...
/*===========================================================================*/
uint16_t addoption(uint8_t * shb, uint16_t optioncode, uint16_t optionlen,
		   char *option)
{
	uint16_t padding;
	      ...
	      uint64_t incomming)
{
	...
	isbhdr->starttime_timestamp_low =
	    (uint32_t) starttimestamp & 0xffffffff;
...
bool writeidb(int fd, uint8_t * macorig, char *interfacestr)
{
	...
	idblen +=
	    addoption(idb + idblen, IF_NAME, strlen(interfacestr),
		      interfacestr);
	memset(&vendor, 0, 6);
...
/*===========================================================================*/
bool writeshb(int fd, uint8_t * macmyap, uint64_t rcrandom,
	      uint8_t * anoncerandom, uint8_t * macmysta)
{
	int shblen;
	int written;
	...
	if (uname(&unameData) == 0) {
		shblen +=
		    addoption(shb + shblen, SHB_HARDWARE,
			      strlen(unameData.machine), unameData.machine);
		sprintf(sysinfo, "%s %s", unameData.sysname, unameData.release);
		shblen +=
		    addoption(shb + shblen, SHB_OS, strlen(sysinfo), sysinfo);
		sprintf(sysinfo, "wifidog %s", VERSION);
		shblen +=
		    addoption(shb + shblen, SHB_USER_APPL, strlen(sysinfo),
			      sysinfo);
	}
	shblen +=
	    addoption(shb + shblen, OPTIONCODE_MACMYAP, 6, (char *) macmyap);
	...
	shblen += 12;
	shblen +=
	    addoption(shb + shblen, OPTIONCODE_ANONCE, 32,
		      (char *) anoncerandom);
	shblen +=
	    addoption(shb + shblen, OPTIONCODE_MACMYSTA, 6, (char *) macmysta);
...
/*===========================================================================*/
int hcxcreatepcapngdump(char *pcapngdumpname, uint8_t * macorig,
			char *interfacestr, uint8_t * macmyap,
			uint64_t rcrandom, uint8_t * anoncerandom,
			uint8_t * macmysta)
{
	int fd;

	umask(0);
	fd = open(pcapngdumpname, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (fd == -1) {
		return -1;
	}
